...
This roadmap outlines the 20-week implementation plan for the Nexus enterprise agent framework.

## Vision

**Nexus** is a Rust-powered, config-driven agent framework that combines:
...
- ✅ `nexus-config`: Full YAML/TOML parsing and validation
- ✅ `nexus-cli`: Working CLI with `init` and `validate` commands
...
- [x] CLI tool (`nexus init`, `nexus validate`)
...
**Tasks**:
- [ ] Design LLM adapter trait in `nexus-llm`
...
**Tasks**:
- [ ] Design tool system in `nexus-tools`
  - Tool trait
...
**Tasks**:
- [ ] Design agent engine in `nexus-runtime`
  - Execution context
  - Message handling
  - Tool calling loop
...
nexus run agent.yaml --input "What's 25 * 47?"
```
...
**Tasks**:
- [ ] Design protocol buffers in `nexus-api`
...
**Tasks**:
- [ ] Implement `nexus run`
- [ ] Implement `nexus logs`
- [ ] Implement `nexus metrics`
- [ ] Shell completions
...
- [ ] **Config Diff Command**: `nexus diff old.yaml new.yaml`
  - Show what changed between config versions
  - Highlight impact of changes

- [ ] **Dry Run Mode**: `nexus run agent.yaml --dry-run`
  - Validate config without calling LLM
  - Show estimated cost and token usage
  - Preview execution plan

- [ ] **CLI Config Override**: `nexus run agent.yaml --override llm.temperature=0.0`
...
- [ ] **Watch Mode**: `nexus run agent.yaml --watch`
...
- [ ] **Run as Daemon**: `nexus run agent.yaml --daemon`
...
- [ ] **Stop Commands**:
  ```bash
  nexus stop <agent-name>              # Stop specific agent
  nexus stop --all                     # Stop all agents
  nexus restart <agent-name>           # Restart agent
  nexus kill <agent-name>              # Force kill (SIGKILL)
  ```

- [ ] **Status Management**:
  ```bash
  nexus status                         # List all running agents
  nexus status <agent-name>            # Detailed status
  nexus logs <agent-name> --follow     # Tail logs
  nexus ps                             # Process list (like docker ps)
  ```
...
nexus run agents/support-agent.yaml --daemon
# Output: Agent 'support-agent' started (PID: 12345)

# Check status
nexus status support-agent
# Output:
#   Name: support-agent
#   Status: Running
#   PID: 12345
...
nexus restart support-agent
# Output: Agent 'support-agent' restarted (PID: 12389)

# Stop when done
nexus stop support-agent
...
| Metric | Nexus Target | Agno | OpenAI AgentKit | LangChain |
|--------|--------------|------|-----------------|-----------|
...
1. `nexus-core/nexus-config/src/parser.rs` - Config parsing ✅
2. `nexus-core/nexus-llm/src/anthropic.rs` - First LLM adapter 🚧 NEXT
3. `nexus-core/nexus-tools/src/http.rs` - First built-in tool
4. `nexus-core/nexus-runtime/src/agent/executor.rs` - Core execution loop
5. `nexus-core/nexus-cli/src/commands/run.rs` - Run command

### Phase 2 (API & SDK - Weeks 5-8)
6. `nexus-core/nexus-api/proto/nexus.proto` - API contract
7. `nexus-py/nexus_sdk/client.py` - Python client
8. `nexus-core/nexus-middleware/src/observability.rs` - OpenTelemetry
...
# CLI Usage
nexus init my-agent              # Create new project
nexus validate agent.yaml        # Validate config
nexus run agent.yaml             # Run agent (Week 4+)
nexus deploy agent.yaml          # Deploy to prod (Week 18+)

# Testing
cargo test -p nexus-config       # Test specific crate
...
1. **Create LLM types** (`nexus-llm/src/types.rs`):
   - `LLMRequest`, `LLMResponse`
   - `Message`, `MessageRole`
   - Streaming types

2. **Define LLM trait** (`nexus-llm/src/adapter.rs`):
   ```rust
   #[async_trait]
...
3. **Implement Anthropic** (`nexus-llm/src/anthropic.rs`):
   - Messages API integration
   - Streaming via SSE
   - Error handling
   - Rate limiting
