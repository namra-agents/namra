...
- ✅ **Common Types** (`nexus-llm/src/types.rs`)
...
- ✅ **LLMAdapter Trait** (`nexus-llm/src/adapter.rs`)
...
- ✅ **Anthropic Adapter** (`nexus-llm/src/anthropic.rs`)
...
nexus-core/nexus-cli/
├── Cargo.toml                     (added nexus-llm dependency)
├── src/
...
```bash
$ export ANTHROPIC_API_KEY=sk-ant-...
$ nexus run /tmp/test-week2/agents/example_agent_v2.yaml \
  --input "Say 'Hello from Nexus!' and nothing else"
...
- [x] `nexus run` command functional
